...
<body>
    ...
    try {
        ...
        $conex->setAttribute(PDO::ATTR_CASE, PDO::CASE_LOWER);
    ...
    <div class="contenido">
        ...
        if (isset($_POST["enviar2"])) {
            try {
                $preparada = $conex->prepare("update stock s, tienda t set s.unidades = ? where s.producto = ? and s.tienda = t.cod and t.nombre = ?");
                $result = $conex->query("select nombre from tienda");
                while ($fila = $result->fetchObject()) {
                    if (isset($_POST[$fila->nombre])) {
                        $preparada->bindParam(1, $_POST[$fila->nombre]);
                        $preparada->bindParam(2, $_POST['producto']);
                        $preparada->bindParam(3, $fila->nombre);
